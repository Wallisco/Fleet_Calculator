...
/* ---------- fleet operator savings proposal ---------- */
function buildSavingsPdf(payload) {
  return new Promise((resolve, reject) => {
    ...
    const m = M.computeSavings(payload.inputs);
    ...
    const wm = asset('logo-white.png');
    if (wm) doc.image(wm, L, 24, { width: 132 });
    else doc.font(HB).fontSize(18).fillColor('#FFFFFF').text('ScootHero', L, 26, { lineBreak: false });
    ...
    doc.font(H).fontSize(15).fillColor('#FFFFFF')
      .text('The driver deal, in numbers', L, 68, { lineBreak: false });
    const st = new Date(payload.createdAt || Date.now());
    doc.font(B).fontSize(8).fillColor('#9AA0B4')
      .text(st.toLocaleDateString('en-ZA', { day: '2-digit', month: 'short', year: 'numeric' }),
            L, 27, { width: W, align: 'right', lineBreak: false });
    if (c.company || c.name) doc.font(BB).fontSize(10).fillColor('#FFFFFF')
    ...
    const setup = [
      ['Bikes', String(m.inputs.bikes)],
      ['Petrol price', money(m.inputs.fuelPrice) + ' /l'],
      ['Petrol rental', money(m.inputs.petrolRental)],
      ['Electric rental', money(m.inputs.electricRental)]
    ];
    const sw = W / 4;
    setup.forEach((s2, i) => {
      const cx = L + i * sw + 14;
      doc.font(B).fontSize(6.5).fillColor(MUTED)
        .text(s2[0].toUpperCase(), cx, y + 13, { width: sw - 20, characterSpacing: 0.5, lineBreak: false });
      doc.font(BB).fontSize(11).fillColor(NAVY).text(s2[1], cx, y + 26, { width: sw - 20 });
    });
    y += 78;

    /* verdict */
    const ok = m.driverBetterOff;
    doc.roundedRect(L, y, W, 54, 9).fill(ok ? LEAF : WARN);
    doc.font(HB).fontSize(11).fillColor(ok ? '#1E5B2A' : WARN_INK)
      .text(ok ? 'The deal works' : 'The deal does not work at this petrol price', L + 16, y + 12, { lineBreak: false });
    doc.font(B).fontSize(8.5).fillColor(ok ? '#1E5B2A' : WARN_INK).text(
      ok
        ? 'Your rider is ' + money(m.driverSavingWk) + ' a week better off on electric while you collect ' +
          money(m.rentalGap) + ' more rental. You could charge up to ' + money(m.breakEvenRental) + '.'
        : 'Your rider would be ' + money(Math.abs(m.driverSavingWk)) + ' a week worse off. Drop the electric rental to ' +
          money(m.breakEvenRental) + ' or below to make the switch worth their while.',
      L + 16, y + 28, { width: W - 32 });
    y += 68;

    /* the rider's week */
    doc.font(HB).fontSize(13).fillColor(NAVY).text("The rider's week", L, y, { lineBreak: false });
    doc.font(B).fontSize(8.5).fillColor(MUTED)
      .text(M.moneyC(m.petrolPerKm) + ' a km on petrol at ' + money(m.inputs.fuelPrice) + '/l and ' + m.inputs.kmPerLitre + ' km/l',
            L, y + 3, { width: W, align: 'right', lineBreak: false });
    y += 20;
    const cw = [W * 0.24, W * 0.16, W * 0.20, W * 0.22, W * 0.18];
    ['Day', 'Km', 'Petrol fuel', 'Electric energy', 'Saved'].forEach((h, i) => {
      const hx = L + cw.slice(0, i).reduce((a, b) => a + b, 0);
      doc.rect(hx, y, cw[i], 18).fill(SURFACE);
      doc.font(B).fontSize(6).fillColor(MUTED).text(h.toUpperCase(), hx + 5, y + 6,
        { width: cw[i] - 10, align: i ? 'right' : 'left', characterSpacing: 0.4, lineBreak: false });
    });
    y += 18;
    m.rows.forEach(r => {
      [r.day, r.km.toFixed(0), money(r.petrol), money(r.electric), money(r.saving)].forEach((v, i) => {
        const hx = L + cw.slice(0, i).reduce((a, b) => a + b, 0);
        doc.font(B).fontSize(8.5).fillColor(NAVY).text(v, hx + 5, y + 5,
          { width: cw[i] - 10, align: i ? 'right' : 'left', lineBreak: false });
      });
      y += 18;
      doc.moveTo(L, y).lineTo(L + W, y).lineWidth(0.5).stroke(LINE);
    });
    doc.rect(L, y, W, 20).fill('#FFF4EC');
    ['Week', m.weeklyKm.toFixed(0), money(m.petrolFuelWk), money(m.electricEnergyWk), money(m.energySaving)]
      .forEach((v, i) => {
        const hx = L + cw.slice(0, i).reduce((a, b) => a + b, 0);
        doc.font(BB).fontSize(9).fillColor(NAVY).text(v, hx + 5, y + 6,
          { width: cw[i] - 10, align: i ? 'right' : 'left', lineBreak: false });
      });
    y += 34;

    /* what it is worth to the operator */
    need(180);
    const kp = [
      ['Extra rental a week', money(m.operatorUpliftWk)],
      ['Extra rental a month', money(m.operatorUpliftMonth)],
      ['Rider keeps a week', money(m.driverSavingWk)],
      ['Max rental', money(m.breakEvenRental)]
    ];
    const panelH = 57 + 64 + 72 + 14 + 4 * 14 + 30;
    doc.roundedRect(L, y, W, panelH, 10).fill(NAVY);
    let py = y + 19;
    doc.font(HB).fontSize(13).fillColor('#FFFFFF')
      .text(m.inputs.bikes + (m.inputs.bikes === 1 ? ' bike' : ' bikes'), L + 18, py, { lineBreak: false });
    py += 17;
    doc.font(B).fontSize(8.5).fillColor('#9AA0B4')
      .text('What the switch is worth to you', L + 18, py, { lineBreak: false });
    py += 21;
    const kw = (W - 36 - 27) / 4;
    kp.forEach((k, i) => {
      ...
      doc.font(H).fontSize(6).fillColor(MINT)
        .text(k[0].toUpperCase(), kx + 10, py + 11, { width: kw - 20, characterSpacing: 0.5 });
      doc.font(HB).fontSize(13).fillColor('#FFFFFF')
        .text(k[1], kx + 10, py + 27, { width: kw - 20, lineBreak: false });
    });
    py += 64;
    doc.roundedRect(L + 18, py, W - 36, 60, 8).fill(ORANGE);
    doc.font(H).fontSize(9).fillColor('#FFFFFF')
      .text('EXTRA RENTAL A YEAR', L + 34, py + 15, { characterSpacing: 1.2, lineBreak: false });
    doc.font(B).fontSize(9.5).fillColor('#FFE9D8')
      .text('across your fleet, on the same riders', L + 34, py + 31, { lineBreak: false });
    doc.font(HB).fontSize(20).fillColor('#FFFFFF')
      .text(money(m.operatorUpliftYear), L + 18, py + 22, { width: W - 52, align: 'right', lineBreak: false });
    py += 72;
    doc.font(H).fontSize(7).fillColor('#FFFFFF')
      .text('PER BIKE, PER WEEK', L + 18, py, { characterSpacing: 1, lineBreak: false });
    py += 14;
    [['Petrol fuel', money(m.petrolFuelWk)],
     ['Electric energy', '-' + money(m.electricEnergyWk)],
     ['Extra rental you charge', '-' + money(m.rentalGap)],
     ['Rider still ahead by', money(m.driverSavingWk)]].forEach((r, i) => {
      const last = i === 3;
      doc.font(last ? BB : B).fontSize(8.5).fillColor(last ? '#FFFFFF' : '#C4C9DA')
        .text(r[0], L + 18, py, { lineBreak: false });
      doc.font(BB).fontSize(8.5).fillColor(last ? MINT : '#FFFFFF')
        .text(r[1], L + 18, py, { width: W - 54, align: 'right', lineBreak: false });
      py += 14;
    });
    y += panelH + 22;
    ...
    /* what you stop doing */
    need(200);
    doc.font(HB).fontSize(15).fillColor(NAVY).text('What you stop doing', L, y, { lineBreak: false });
    y += 20;
    doc.font(B).fontSize(8.5).fillColor(MUTED).text(
      'The rand saving is only half of it. Most of what makes a fleet exhausting to run is not on the invoice.',
      L, y, { width: W - 40 });
    y += 20;

    const bens = [
      ['No late-night callouts', 'A rider breaks down at nine at night and it is our phone that rings. Roadside assistance runs day and night as part of HeroCare.'],
      ['No chasing tyres and punctures', 'Tyres, punctures, brakes and services are all scheduled and done. You never phone around for a fitment slot again.'],
      ['No lying awake about theft', 'Every bike is tracked from handover, with recovery and an SOS line behind it. Theft and hijacking stop being your exposure.'],
      ['No admin and licensing pile', 'Registration, licensing and the paperwork of a growing fleet are handled. Your time goes to the business, not the queue.'],
      ['No hunting for riders', 'Take vetted, delivery-experienced riders from our marketplace. If someone leaves we refill the seat rather than parking the bike.'],
      ['No surprise repair bills', 'Bikes are fixed before they fail on our predictive maintenance schedule, and insurance sits inside the deal protecting your asset.']
    ];
    const bg = 10, bw = (W - bg) / 2;
    for (let i = 0; i < bens.length; i += 2) {
      const pair = bens.slice(i, i + 2);
      const bh = Math.max.apply(null, pair.map(r =>
        24 + doc.font(B).fontSize(7.5).heightOfString(r[1], { width: bw - 26 }) + 10));
      need(bh + 12);
      pair.forEach((r, j) => {
        const bx = L + j * (bw + bg);
        doc.roundedRect(bx, y, bw, bh, 8).fillAndStroke('#FFFFFF', LINE);
        doc.rect(bx, y + 12, 2.5, bh - 24).fill(MINT);
        doc.font(BB).fontSize(9).fillColor(NAVY).text(r[0], bx + 14, y + 11, { width: bw - 26, lineBreak: false });
        doc.font(B).fontSize(7.5).fillColor(MUTED).text(r[1], bx + 14, y + 24, { width: bw - 26 });
      });
      y += bh + 10;
    }
    y += 10;
    ...
    y += 22;
    [['Sign off this proposal', 'Confirm the rider deal on the last page and send it back. That is the only approval we need.'],
     ['We agree your rider terms', 'We set your electric rental in Lighthouse and brief your riders on what changes for them.'],
     ['Receive your invoice', 'For the bikes you are taking on, with VAT added.'],
     ['Pay the invoice', 'Payment confirms your build slot and starts the delivery clock.'],
     ['Delivery within 7 days', 'Bikes arrive inspected, registered and ready to ride.'],
     ['Riders switch and go live', 'We train your riders, orient them on the swap network, and your new rental takes effect.']
    ].forEach((st2, i) => {
      const h = 20 + doc.font(B).fontSize(8.5).heightOfString(st2[1], { width: W - 62 });
      if (y + h + 8 > BOTTOM) { doc.addPage(); y = 50; }
      doc.circle(L + 11, y + 10, 11).fill(NAVY);
      doc.font(HB).fontSize(9).fillColor('#FFFFFF').text(String(i + 1), L, y + 6.5, { width: 22, align: 'center', lineBreak: false });
      doc.font(BB).fontSize(10.5).fillColor(NAVY).text(st2[0], L + 32, y + 1, { width: W - 42, lineBreak: false });
      doc.font(B).fontSize(8.5).fillColor(MUTED).text(st2[1], L + 32, y + 15, { width: W - 62 });
      y += h + 12;
    });
    ...
    doc.font(B).fontSize(9).fillColor(MUTED).text(
      'Confirm the rider deal below, then sign. Return this page to your ScootHero contact and we will set it up.',
      L, y, { width: W - 40 });
    y += 30;
    [['Bikes', String(m.inputs.bikes)],
     ['Weekly rental you will charge each rider', money(m.inputs.electricRental)],
     ['Extra rental across the fleet, a month', money(m.operatorUpliftMonth)],
     ['Your rider stays ahead by, a week', money(m.driverSavingWk)]].forEach(o => {
      doc.roundedRect(L, y, W, 34, 8).fillAndStroke('#FFFFFF', LINE);
      doc.font(B).fontSize(9.5).fillColor(NAVY).text(o[0], L + 16, y + 12, { lineBreak: false });
      doc.font(BB).fontSize(10.5).fillColor(NAVY).text(o[1], L + 16, y + 11, { width: W - 32, align: 'right', lineBreak: false });
      y += 40;
    });
    y += 8;
    const half = (W - 20) / 2;
    [['Full name', ''], ['Capacity (e.g. Director)', ''], ['Company', c.company || ''],
     ['Registration number', ''], ['Signature', ''], ['Date', '']].forEach((f, i) => {
      const sx = L + (i % 2) * (half + 20);
      if (i % 2 === 0 && i > 0) y += 46;
      doc.font(H).fontSize(7).fillColor(MUTED).text(f[0].toUpperCase(), sx, y, { characterSpacing: 0.6, lineBreak: false });
      if (f[1]) doc.font(B).fontSize(10).fillColor(NAVY).text(f[1], sx, y + 13, { lineBreak: false });
      doc.moveTo(sx, y + 30).lineTo(sx + half, y + 30).lineWidth(0.75).stroke('#B9BDC9');
    });
    y += 58;
    doc.font(B).fontSize(7.5).fillColor(MUTED).text(
      'By signing, you accept the rider deal set out above. Figures are estimates based on the petrol price and kilometres shown, ' +
      'and will move with the pump price. This is an acceptance of terms and not a credit agreement.',
      L, y, { width: W - 40 });

    /* footer */
    const DISC = 'INDICATIVE ONLY. Built on the assumptions shown and aggregated industry data. Not a forecast, a guarantee of return, ' +
      'or an offer of finance; actual results may be materially different. Nothing here is financial, tax or legal advice. ' +
      'Petrol cost per kilometre is derived from the pump price shown and a fixed 24 km per litre, typical for a ' +
      'loaded delivery bike in traffic. Kilometres are average market figures and vary by route and rider. Excludes maintenance, insurance ' +
      'and tracking. All figures exclude VAT. Savings move with the petrol price. ' + LEGAL.POPIA_SHORT;
    const mark = asset('watermark.png');
    const range = doc.bufferedPageRange();
    for (let p2 = range.start; p2 < range.start + range.count; p2++) {
      doc.switchToPage(p2);
      doc.page.margins.bottom = 0;
      if (mark) { try { doc.image(mark, doc.page.width - 74, doc.page.height - 50, { width: 26 }); } catch (_) {} }
      const fy = doc.page.height - 58;
      doc.moveTo(L, fy).lineTo(L + W, fy).lineWidth(0.5).stroke(LINE);
      doc.font(B).fontSize(6).fillColor(MUTED).text(DISC, L, fy + 6, { width: W - 44, lineGap: 0.5 });
      doc.font(B).fontSize(7).fillColor(MUTED)
        .text((p2 - range.start + 1) + ' / ' + range.count, L, fy + 6, { width: W, align: 'right', lineBreak: false });
    }
    doc.end();
  });
}
...
function buildPdf(payload) {
  if (payload && payload.kind === 'savings') return buildSavingsPdf(payload);
  return buildFleetPdf(payload);
}

function buildFleetPdf(payload) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'A4', margin: 46, bufferPages: true });
    const chunks = [];
    doc.on('data', c => chunks.push(c));
    ...
    /* ---------- header ---------- */
    doc.rect(0, 0, doc.page.width, 104).fill(NAVY);
    const wordmark = asset('logo-white.png');
    if (wordmark) {
      doc.image(wordmark, L, 24, { width: 132 });   // 1000x92 source
    } else {
    ...
    const brRows = m.onRent ? [
      ...
      ['Capital, over the term', '-' + money(m.capitalCharge)]
    ] : [
    ...
    const y3 = M.compute(payload.inputs, m.fund, m.years, 3);
    ...
    doc.font(B).fontSize(9.5).fillColor('#FFE9D8').text(
      m.onRent
        ? 'over ' + m.years + (m.years === 1 ? ' year, on ' : ' years, on ') + money(m.capitalCharge) + ' of capital'
    ...
    [[y1, 'OVER 1 YEAR'], [y3, 'OVER 3 YEARS']].forEach((pair, ci) => {
      ...
      cy += 14;
      (m.onRent ? [
        ['Rent collected', money(r.revM * r.months)],
        ['Running costs', '-' + money(r.opM * r.months)],
        ['Finance', r.finM ? '-' + money(r.commit) : '—'],
        [r.horizon < m.years ? ('Capital, 1 of ' + m.years + ' yrs') : 'Capital, over the term',
         '-' + money(r.capitalCharge)]
      ] : [
        ['HeroCare', money(r.careWk * 4.345 * r.inputs.bikes * r.months)],
        ['Admin fee', money(r.adminWk * 4.345 * r.inputs.bikes * r.months)],
        ['Energy', money(r.fuelWk * 4.345 * r.inputs.bikes * r.months)],
        ['Instalments', money(r.commit)]
      ]).forEach(row => {
    ...
    const risks = [
      ...
      ['A bike is crashed or stolen', 'Insurance, tracking, SOS and recovery are all inside the HeroCare fee. Every bike is tracked from the day it is handed over.', 'You carry a R5 000 excess per claim. One uninsured write-off would cost a year of profit on that bike.'],
    ...
    const DISCLAIMER = m.onRent
      ? 'INDICATIVE ONLY. Projected returns are built on the assumptions shown, including aggregated industry data. ' +
        'They are not a forecast, a guarantee of return, or an offer of finance, and actual results may be materially different. ' +
        'Nothing here is financial, tax or legal advice. Bike, inspection and delivery box prices are firm and hold for 14 days from the date of this proposal; VAT is added at invoicing. ' +
      ...
      : 'INDICATIVE ONLY. Built on the assumptions shown, including aggregated industry data. Not a forecast, a guarantee of ' +
        'return, or an offer of finance. Subject to credit approval through Eqstra, a Nedbank company; the finance ' +
        'rate shown is indicative only. All figures exclude VAT. Running costs vary by area, rider and usage, and exclude tax, admin ' +
        'costs and any residual owing at the end of a lease. Speak to us before making a financial decision. ' + LEGAL.POPIA_SHORT;
    ...
    for (let p = range.start; p < range.start + range.count; p++) {
      doc.switchToPage(p);
      doc.page.margins.bottom = 0;
      // faint brand mark, bottom-right, behind nothing that matters
      if (mark) {
        try {
          // small, in the footer gutter — present but never over content
          doc.image(mark, doc.page.width - 74, doc.page.height - 50, { width: 26 });
        } catch (_) { /* non-fatal */ }
      }
      const fy = doc.page.height - 58;
      doc.moveTo(L, fy).lineTo(L + W, fy).lineWidth(0.5).stroke(LINE);
...
module.exports = { buildPdf, buildSavingsPdf };
