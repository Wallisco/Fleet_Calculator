...
<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden">
  ...
  <div style="padding:18px 26px;background:#F4F5F7">
    <p style="font:11px Helvetica,Arial,sans-serif;color:#5A5F73;line-height:1.5;margin:0">
      Estimates only. Petrol cost per kilometre is derived from the pump price shown and a fixed 24 km per litre.
      Savings move with the pump price. You received this because you asked us to email your calculation.
    </p>
  </div>
</div></body></html>`;
}
...
<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden">
  ...
  <div style="padding:18px 26px;background:#F4F5F7">
    <p style="font:11px Helvetica,Arial,sans-serif;color:#5A5F73;line-height:1.5;margin:0">
      Estimates based on the numbers you put in and average rates a rider was paid over June and July 2026.
      Your own rates and costs may differ. Not an offer of work or credit. You received this because you asked us to send it.
    </p>
  </div>
</div></body></html>`;
}
...
async function sendDriverEmail(contact, m, ref, portalUrl) {
  ...
  if (team.length) {
    await send({
      from, to: team,
      subject: 'Driver lead — ' + (contact.name || contact.email) + ' · ' + money(m.savingMonth) + '/mo better off',
