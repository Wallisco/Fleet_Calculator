...
<h3 style="margin:20px 0 6px;font:bold 14px Helvetica,Arial,sans-serif">Express — income on top, and it moves</h3>
<table style="border-collapse:collapse;width:100%;max-width:520px">
  ...
  ${row('A busy express day runs about', M.EXPRESS.typicalPerDay + ' parcels')}
  ${row('Claim reads as', {
      none: 'No express work — food only',
      mixed: 'Part express, part food',
      busy: 'A busy express day',
      above: 'ABOVE a realistic day — check this'
    }[c.expressClaim])}
  ${row('Riders on express routes', 'About ' + M.EXPRESS.riders)}
</table>
<p style="color:#5A5F73;font:12px Helvetica,Arial,sans-serif;margin:14px 0 0">
  Express is <b>e-commerce driven</b>, so volume rises and falls with what the retailers are shipping. On a
  light express day riders top up with food, which is why express riders already appear in the branch food
  data and why their food count is residual work rather than a measure of how good they are. Their split
  will move week to week; total income is the steadier number. Only about ${M.EXPRESS.riders} riders hold
  express routes, so the claim is checkable by name with the operator.
</p>
</body></html>`;
}
